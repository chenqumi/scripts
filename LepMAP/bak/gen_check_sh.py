...
if len(sys.argv) ==1:
    print("\nUsage: {} <order.lst>".format(sys.argv[0]))
    sys.exit()
...
with open(lst) as fd:
    for line in fd:
        ...
        cmd = cmd + "> map{}_{}.txt &&".format(lg,repeat)

        cmd2 = "{} -cp {} LMPlot ".format(JAVA,LM)
        cmd2 = cmd2 + "map{0}_{1}.txt > lmplot{0}_{1}.dot".format(lg,repeat)

        outfile = "check_lg{}_{}.sh".format(lg,repeat)
        ot = open(outfile,"w")
        ot.write(cmd+"\n")
        ot.write(cmd2+"\n")
        ot.close()
