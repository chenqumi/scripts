...
#
$ref = abs_path($ref);
$bam_lst = abs_path($bam_lst) if (defined $bam_lst);
...
my $tmp_bam_lst = "$dir_merge/tmp_bam.lst";
	
&merge_file($tmp_bam_lst);
...
if (defined $bam_lst){
	...
	`cat tmp $bam_lst > tmp_bam.lst`;
	my $tmp_bam_lst = "$dir_merge/tmp_bam.lst";
	# TODO sub format of bam file!!!!!!!!!!!!!!
	&merge_file_nosplit($tmp_bam_lst);
	#
	# GATK pipe
	#==============
	&GATK_merge();
...
#
sub merge_file{
	my $tmp_bam_lst = shift;
	my (%lib,%lib2);
	open FI,"$tmp_bam_lst" or die $!;
	while (<FI>) {
		chomp;
		my $file = basename($_);
...
sub merge_file_nosplit{
	...
	while (<FI>) {
		chomp;
		my $file = basename($_);
		my ($sample,$header) = &parse_merge_nosplit($file);
...
sub parse_merge_nosplit{
	my $file = shift;

	if ($file =~ /(\S+)\_batch\.bam/)
	{	
		return $1,"$1\_batch";
	}
	elsif($file =~ /(\S+)\_batchN\.bam/)
	{
		return $1,$1;
	}
	elsif($file =~ /(\S+)\.bam/)
